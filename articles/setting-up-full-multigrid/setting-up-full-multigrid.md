...
preconditioner from a real hierarchy of meshes rather than from the matrix
alone, and this note is about how to get that hierarchy and what it is worth.
...
there remove what the fine mesh could not.

The method is a recursion: smooth on the fine mesh, transfer what is left to a
coarser one, solve there with another recursive step, transfer the correction
back, smooth again. Each *level* handles the band of error it can see cheaply,
and the work per unknown stops growing as the model gets bigger.
...
**Algebraic multigrid** (AMG) builds the coarse levels from the matrix. It
inspects the operator's connectivity, groups unknowns that are strongly
coupled, and constructs coarse levels and transfer operators from that grouping
alone. Because it asks nothing of the mesh it works everywhere, which makes it
the sensible default. PETSc's implementation is GAMG, and it is what Underworld
...
guide to the geometry, which is what jumps and steep gradients in material
properties produce. The algebraic method sees a matrix whose couplings are
...
of those is the better bet is not obvious in advance, so it is measured below.
Switching the geometric solver on comes first, and that starts with the mesh.
...
for a coarse level, and making one takes work — so the hierarchy is something
the solver reads rather than something you handle.
...
rebuilding it is the subject of a
[related technical note](/moving-the-mesh-without-remaking-it/).
...
Both preconditioners are run at PETSc's own defaults, and GAMG is told that the
velocity field has two components per node so that it aggregates nodes rather
than scalars.
...
So the geometric hierarchy is worth roughly a factor of two: FMG does about
half GAMG's arithmetic on this problem, and holds that through six orders of
viscosity contrast and a 250-fold growth in problem size.
...
The difference is in the shape of the arithmetic rather than the amount of it.
Geometric multigrid on an unstructured hierarchy works through sparse, indirect
operators, and its coarse levels are small enough that they cannot keep the
processor busy. Algebraic multigrid does more arithmetic, but in denser and
more regular operators that run closer to the machine's peak rate. On this
hardware the two effects very nearly cancel. The flop count is a property of
the method and should carry to another machine; the rate is a property of this
one, and on a machine with a different memory system the wall-clock comparison
